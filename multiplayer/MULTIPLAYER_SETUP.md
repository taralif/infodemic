...
## 9/20/2026 — weak-clue reasons + the v12 mapper (build "multiplayer catches up")
- `flagReasons {clipId: reasonId}` (gain | one_story | no_proof | other) rides beside `flags` on each player's own r3 slice (`players.slice.r3[claimId]`). No schema change — it is inside the existing per-player JSONB slice. A teammate's reason is read from their slice; anyone's flag keeps a clue out of every note.
- The page now loads `multiplayer/drop-to-claims-v12.js`. A published feed whose cards all carry `post.planted` (generator v12/v13) goes through `DropToClaimsV12.mapDropCards`, which also strips the authoring fields `strength` / `weak_reason` before anything renders. Older feeds (v10/v11 rows) fall back to the built-in five with a console warning.
